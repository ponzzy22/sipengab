...
<div class="col-md-12">
  <div class="box">
	  ...
	  foreach($records as $rec) {
        ...
        <tr>
		  ...
		  <td><?php if($status == "active") {?>
            <a href="javascript:status('<?php echo $user_id ?>', 'inactive');">Nonaktif</a>&nbsp;/
			&nbsp;<a href="javascript:status('<?php echo $user_id ?>', 'lock');">Lock</a>&nbsp;/
			&nbsp;<a href="javascript:status('<?php echo $user_id ?>', 'delete');">Hapus</a>
            <?php } else { ?>
			<a href="javascript:status('<?php echo $user_id ?>', 'active');">Aktif</a>
			<?php }//else ?>
          </td>
		  <?php }?>
        </tr>
	...
	if($type == 'admin') {
	?>
	<button type="button" class="btn btn-info" onclick="javascript:createUserForm();"><i class="fa fa-user-plus" aria-hidden="true"></i>&nbsp;Tambah User</button>
	<?php 
	}
	?>
      <!--
